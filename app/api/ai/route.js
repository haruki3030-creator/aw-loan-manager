...
6. ★근저당변경(채권최고액 변경): "N-M번근저당권변경"이 있으면 N번 근저당의 채권최고액이 변경된 것. 반드시 변경 후 금액을 적용!
   예) 4번 근저당 원래 168,000,000원 → 4-3에서 123,200,000원으로 변경 → 최종 채권최고액은 12,320만
7. ★대환/선순위 자동 판별 (등기부 기준):
   - 카톡에 "선순위"/"대환" 구분이 명시되어 있으면 그것을 따름
   - 카톡에 구분이 없고 등기부에 근저당이 여러 건일 때:
     a) 카톡 요청서에 선순위로 기재된 대출만 seniorLoans
     b) 카톡에 언급되지 않은 등기부 근저당(대부업체 등)은 대환 가능성 검토
     c) 근저당권자가 대부업체(XX대부, XX파이낸셜 등)이고, 카톡 선순위 목록에 없으면 → replacementLoans로 분류
   - 판단 불확실하면 special 필드에 "등기부상 [업체명] [금액] 대환여부 확인 필요" 기재` : ""}
