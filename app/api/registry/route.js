...
8. ★ 신탁이 "신탁재산의귀속"으로 해제되었거나 이후 매매가 있으면 현재 신탁 아님.
9. ★★ 근저당변경 (매우 중요): "N-M번근저당권변경"이 있고 채권최고액이 변경되었으면, 반드시 변경 후 금액을 최종 채권최고액으로 적용!
   예: 4번 근저당 원래 168,000,000원, 4-3에서 "채권최고액 금123,200,000원" → 최종 12,320만원
10. ★★ 선순위 vs 대환대상 구분 (근저당이 여러 건일 때):
   - 은행(국민/신한/우리/하나/농협/기업/SC 등) 근저당 → 일반적으로 선순위(유지)
   - 대부업체(XX대부, XX파이낸셜, XX캐피탈 등) 근저당 → 대환 가능성 높음, "대환 가능성" 표기
   - 채권양도 이력(N-M번근저당권이전)이 여러 차례 있는 근저당 → 고금리 대환 대상일 가능성 매우 높음
   - 유효 근저당 목록에 "(대환가능)" 또는 "(유지)" 표기 추가${cancelledInfo}${hintBlock}
...
🏦 유효 근저당: (말소 안 된 것만. 근저당변경 있으면 변경 후 금액. 각 항목에 (유지)/(대환가능) 표기)${kb ? ` LTV 포함` : ""}
✅ 종합 판단: 안전/보통/주의/위험 + 사유 + 후순위 가능여부 (유지 선순위 기준 LTV로 판단. 대환대상은 실행 시 말소되므로 LTV 제외)
